...
public class PrintUtils {
    ...
    public PrintUtils(int fb, String printType) {
        this.fb = fb;
    ...
    */
    ...
    public static final byte[] ESC_F = new byte[]{ESC, 0x63, 0x01};
    //DLE EOT n 实时状态传送
    //如果返回结果为22
    /**
     * 、DLE EOT n 实时状态传送
    ...
    public void initPrinter() {
        ...
        //ESC_INIT 将在清空缓存区的数据
//        UartOptNative.uartSendMessageNative(fb, ESC_INIT);
//        //自定义字体
//        UartOptNative.uartSendMessageNative(fb, ESC_FS_2);
//        UartOptNative.uartSendMessageNative(fb, ESC_STANDARD);
//        UartOptNative.uartSendMessageNative(fb, ESC_CANCEL_DEFINE_FONT);
//        UartOptNative.uartSendMessageNative(fb, ESC_FONTA);
//        UartOptNative.uartSendMessageNative(fb, ESC_SELECT_CHARACTER);
//
//        //进入汉字模式打印
//        UartOptNative.uartSendMessageNative(fb, ESC_CN_FONT);
//        UartOptNative.uartSendMessageNative(fb, ESC_FONT_B);
//        UartOptNative.uartSendMessageNative(fb, ESC_FONTA);
    }
    ...
    public void executeLineFeedAndPaperCut() {
        UartOptNative.uartSendMessageNative(fb, POS_CUT_MODE_PARTIAL);
    }
    ...
    public void billHeaderPrinter(String str) {
        UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_CENTER);
        UartOptNative.uartSendMessageNative(fb, FS_FONT_DOUBLE);

        try {
            byte[] tmp = (str + "\n").getBytes("GBK");
            UartOptNative.uartSendMessageNative(fb, tmp);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        UartOptNative.uartSendMessageNative(fb, LF);
    }
    ...
    public void callNumPrinter(String str) {
        UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_LEFT);
        UartOptNative.uartSendMessageNative(fb, FS_FONT_DOUBLE);
        try {
            UartOptNative.uartSendMessageNative(fb, (str + "\n").getBytes("GBK"));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
    }
     ...
     * @param str
     */
    public void doubleSizePrinter(String str, String align) throws UnsupportedEncodingException {
        if (CENTER.equals(align)) {
            UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_LEFT);
        } else if (RIGHT.equals(align)) {
            UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_RIGHT);
        } else {
            UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_LEFT);
        }
        UartOptNative.uartSendMessageNative(fb, FS_FONT_DOUBLE);
    ...
    public void standardPrinterLine(String str, String align) {
        try {
            if (CENTER.equals(align)) {
                UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_CENTER);
                UartOptNative.uartSendMessageNative(fb, FS_FONT_ALIGN);
                UartOptNative.uartSendMessageNative(fb, ESC_CN_FONT);
                UartOptNative.uartSendMessageNative(fb, ESC_CANCEL_BOLD);
                if ("1".equals(printType)) {
                    UartOptNative.uartSendMessageNative(fb, ESC_FONTA);
                } else {
                    UartOptNative.uartSendMessageNative(fb, ESC_FONT_B);
                }
                UartOptNative.uartSendMessageNative(fb, str.getBytes("GBK"));
            } else if (RIGHT.equals(align)) {
                UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_RIGHT);
                UartOptNative.uartSendMessageNative(fb, FS_FONT_ALIGN);
                UartOptNative.uartSendMessageNative(fb, ESC_CN_FONT);
            ...
            UartOptNative.uartSendMessageNative(fb, "\n".getBytes("GBK"));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
    }
     ...
     * @param align 打印的位置 LEFT/CENTER/RIGHT 其他为默认居左打印
     */
    public void standardBoldPrinterLine(String str, String align) throws UnsupportedEncodingException {
        if (CENTER.equals(align)) {
            UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_CENTER);
            UartOptNative.uartSendMessageNative(fb, FS_FONT_ALIGN);
            UartOptNative.uartSendMessageNative(fb, ESC_CN_FONT);
            UartOptNative.uartSendMessageNative(fb, ESC_SETTING_BOLD);
            if ("1".equals(printType)) {
                UartOptNative.uartSendMessageNative(fb, ESC_FONTA);
            } else {
                UartOptNative.uartSendMessageNative(fb, ESC_FONT_B);
     ...
     * @param align
     */
    public void largeSizePrinterLine(String str, String align) throws UnsupportedEncodingException {
        if (CENTER.equals(align)) {
            UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_CENTER);
            UartOptNative.uartSendMessageNative(fb, FS_FONT_ALIGN_DOUBLE);
            UartOptNative.uartSendMessageNative(fb, str.getBytes("GBK"));
        } else if (RIGHT.equals(align)) {
            UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_RIGHT);
            UartOptNative.uartSendMessageNative(fb, FS_FONT_ALIGN_DOUBLE);
     ...
     * @param align
     */
    public void largeHSizePrinterLine(String str, String align) throws UnsupportedEncodingException {
        if (CENTER.equals(align)) {
            UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_CENTER);
            UartOptNative.uartSendMessageNative(fb, FS_FONT_VERTICAL_DOUBLE);
            UartOptNative.uartSendMessageNative(fb, str.getBytes("GBK"));
        } else if (RIGHT.equals(align)) {
            UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_RIGHT);
            UartOptNative.uartSendMessageNative(fb, FS_FONT_VERTICAL_DOUBLE);
     ...
     * @param align
     */
    public void largeSizeRedPrinterLine(String str, String align) throws UnsupportedEncodingException {
        if (CENTER.equals(align)) {
            UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_CENTER);
            UartOptNative.uartSendMessageNative(fb, FS_FONT_ALIGN_DOUBLE);
            UartOptNative.uartSendMessageNative(fb, ESC_FONT_COLOR_RED);
            UartOptNative.uartSendMessageNative(fb, str.getBytes("GBK"));
        } else if (RIGHT.equals(align)) {
            UartOptNative.uartSendMessageNative(fb, ESC_ALIGN_RIGHT);
            UartOptNative.uartSendMessageNative(fb, FS_FONT_ALIGN_DOUBLE);
    ...
    public void openDrawer() {
        UartOptNative.uartSendMessageNative(fb, ESC_OPEN_DRAWER);
    }
